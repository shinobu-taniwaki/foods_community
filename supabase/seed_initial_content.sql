...
BEGIN
  ...
  SELECT admin_id, 'column',
    ...
    'ログイン画面の「パスワードをお忘れの方」から、登録したメールアドレスを入力してください。' || E'\n\n'
    || '再設定用のメールが届きますので、メールの中のボタンから新しいパスワードを設定できます。' || E'\n\n'
    || 'メールが届かない場合は、迷惑メールフォルダもご確認ください。それでも見つからないときは、お問い合わせフォームからご連絡ください。',
    false, 'published', now()
  WHERE NOT EXISTS (
  ...
  SELECT admin_id, 'column',
    ...
    || '申し込みフォームを送信いただくと、運営が確認のうえ変更のお手続きをします。変更が完了すると、お知らせでご連絡します。' || E'\n\n'
